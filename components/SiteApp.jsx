...
function StatCallout({ config = {} }) {
  ...
  return (
    ...
    >
      <div
        className={`${statClass} font-bold tracking-tight`}
        style={{ color: "#c7d2fe" }}
      >
        {stat}
      </div>
      ...
      <div className="relative flex justify-between items-start gap-3">
        <div
          ...
          style={{
            ...
            height: 2,
            background:
              "linear-gradient(90deg, rgba(99,102,241,0.15), rgba(99,102,241,0.55), rgba(99,102,241,0.15))",
          }}
          aria-hidden="true"
        />
        {events.map((event, i) => (
...
function PathwaysGrid() {
  ...
  const entries = [
    { key: "land", label: "Land", accent: ACCENT_SLATE, data: p.land },
    { key: "underwater", label: "Underwater", accent: ACCENT_CYAN, data: p.underwater },
    { key: "orbital", label: "Orbital", accent: ACCENT_INDIGO, data: p.orbital },
  ];
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {entries.map((e) => (
        <div
          key={e.key}
          className="card p-5"
          style={{ borderTop: `4px solid ${e.accent}` }}
        >
          ...
          <span
            className="inline-block px-3 py-1 rounded-full text-xs lg:text-sm font-semibold text-slate-100"
...
function StageComparisonMatrix() {
  ...
  return (
    <div className="card p-0 overflow-hidden">
      <table className="w-full text-sm lg:text-base" style={{ borderCollapse: "collapse" }}>
        <thead>
          <tr className="bg-slate-900/60 text-slate-300">
            <th className="text-left p-3 lg:p-4 font-semibold border-b border-slate-800">
              {m.axisHeader}
            </th>
            {m.columns.map((c) => (
              <th
                key={c}
                className="text-left p-3 lg:p-4 font-semibold border-b border-slate-800"
              >
                {c}
              </th>
            ))}
        ...
        <tbody>
          {m.rows.map((row) => (
            <tr key={row.axis} className="border-b border-slate-800">
              <td className="p-3 lg:p-4 text-slate-200 font-medium align-top">
                {row.axis}
              </td>
              <td className="p-3 lg:p-4 text-slate-300 align-top">{row.land}</td>
              <td className="p-3 lg:p-4 text-slate-300 align-top">
                {row.underwater}
              </td>
              <td className="p-3 lg:p-4 text-slate-300 align-top">{row.orbital}</td>
            </tr>
          ))}
        </tbody>
      </table>
